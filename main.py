...
API_ID = 29152063
API_HASH = "df3207f0fe395c1a79919d7eb311c8e4"
BOT_TOKEN = "5463230148:AAFn1tZbw6q_ewGfTls1UAEvnrcttqu5p7g"
AUTH_USERS = 5760012562
sudo_users = [5760012562]
GROUPS = -1001759789467
bot = Client(
    "bot",
...
async def account_login(bot: Client, m: Message):
	...
	try:    
		...
		os.remove(x)
		return
	editable = await m.reply_text(f"Total Links are in this txt**{len(links)}**\n\nSend From where you want to download initial is **0**")
	...
	try:
		...
		arg = 0
	editable = await editable.edit("**Enter TITLE Name**")
...
async def account_login(bot: Client, m: Message):
	...
	editable = await m.reply_text(
		f"Total Links Found in this Txt **{len(links)}**\n\nSend From where you want to download initial is **0**"
	)
	input1: Message = await bot.listen(editable.chat.id)
	raw_text = input1.text
	...
	try:
		for i in range(arg, len(links)):
			...
			try:
				Show = f"**Downloading:-**\n\n**Name :-** `{name}`\n**Quality -** `{raw_text2}`\n\n**Url :-** `{url}`"
				prog = await m.reply_text(Show)
				cc = f'{str(count).zfill(3)}**.** `{name1} {res}`\n**TITLE :-** `{raw_text0}`\n**Resolution :-** `{raw_text2}`\n**Download by :-** `Anonymous Admin`\n\n**<i>📚💪 Unlock your potential through education.</i>**'
